...
df <- subset(safety, select = -c(Case,No,Task,segment,year,miles,milesa,night,nighta,ppark,gender,age,agea,educ,region,Urb,income,incomea,Ch1,Ch2,Ch3,Ch4))
head(df)
```
...
write.csv(df, file = "train1_preprocessed.csv", row.names = FALSE)
```

# Test Data
## Import Test Data
```{r}
safety <- read.csv("test1.csv")
str(safety)
head(safety)
```
## Add Choice
...
```{r}
# Writing the output to a csv file
write.csv(df, file = "test1_preprocessed.csv", row.names = FALSE)
```
