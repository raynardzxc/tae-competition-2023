...
test <- subset(test, select = -c(Case, No))
```

## Everything model to find important variables
```{r}
set.seed(100)
...
## MeanGiniDecrease > 145
```{r}
set.seed(100)
m2_1<- randomForest(as.factor(Choice)~ Task+BU1+Price1+BU2+Price2+BU3+Price3+yearind+milesa+nighta+agea+Urbind+incomea, data = train, ntree = 500)
predicted <- predict(m2_1, newdata=test, type = "prob")
actual <- test
colnames(predicted) <- c("Ch1", "Ch2", "Ch3", "Ch4")
