...
totrain <- subset(totrain, select = -c(Ch1, Ch2, Ch3, Ch4))
...
## planting my trees
set.seed(100)
inds <- partition(totrain$Choice, p = c(train = 0.7, test = 0.3))
train <- totrain[inds$train, ]
test <- totrain[inds$test, ]

```

## Preparing topredict for later
```{r}
predict_set <- topredict
```
...
full_train <- dfidx(totrain, shape="wide", choice="Choice", sep="", varying = c(4:83), idx = c("No", "Case"))
full_train <- as.data.frame(full_train)
full_train <- subset(full_train, select = -c(idx))
