...
```{r}
safety <- subset(safety, select = -c(CC4, BU4, RP4, LD4, BZ4, FC4, PP4, KA4, SC4, TS4, NV4, Price4, SC3, MA2, FP1, FP3, FP2, SC2, MA3, NS3, NS1, NS2, BU1, BU2, BU3, Urbind, ageind, AF3, LD2, BZ2, CC3, TS2, BZ1, BZ3, PP2, AF1, LD3, LB3, LB2, LB1, MA1, SC1, FA3, KA1, KA2, HU2, NV2, NV1, NV3, TS3, CC2, CC1, PP3, TS1, PP1, GN2, GN1, RP1, FC3, FC1, RP2, FC2, RP3, FA1, HU1, HU3, FA2, KA3, GN4, HU4, AF4, LB4, MA4, FP4, FA4, NS4, GN4, milesind, AF2, LD1, pparkind, regionind, segmentind, educind, nightind, GN3, genderind))
...
```{r}
set.seed(seed)
rf <-randomForest(Choice~.,data=trainingSet, mtry=best.m, importance=TRUE,ntree=1000)
print(rf)
...
```
```{r}
length(testSet$Choice)
length(pred)
```

```{r}
cfn_table <- table(testSet$Choice, pred)
cfn_table
```
```{r}
