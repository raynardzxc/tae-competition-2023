...
```{r}
set.seed(seed)
trainingIndex <- createDataPartition(safety$Choice, p = 0.7, list = FALSE)
...
```{r}
set.seed(seed)
model <- randomForest(as.factor(Choice)~., data = trainingSet, mtry=best.m, importance=TRUE)
model
```
...
```{r}
write.csv(final_predict_df, file = "./Rforest_tryingMyBest.csv", row.names = FALSE)
```
