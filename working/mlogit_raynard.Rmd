...
```{r}
safety <- read.csv("../input/train1_mlogit.csv")
head(safety)
```
...
```{r}
training<-dfidx(subset(safety, Task<=15), shape="wide", choice="Choice", varying =c(4:83), sep="",idx = list(c("No", "Case")))
head(training)
```
```{r}
...
# Usage
safety_names_concatenated <- concatenate_column_names(training)
...
M1 <- mlogit(Choice~GN + FA + LD + BZ + FC + FP + RP + PP + SC + TS + NV + LB + AF + HU + Price | yearind + milesind + nightind + genderind + ageind + educind + regionind + Urbind + incomeind + 0, data=training, print.level=TRUE)
```
...
```{r}
predict_data <- read.csv("../input/test1_onehot.csv")
...
prediction <- predict(M3, newdata=predict_set)
