...
```{r}
# drop unused columns
# df <- subset(safety, select = -c(segmentind,yearind,milesa,milesind,nighta,nightind,pparkind,genderind,agea,ageind,educind,regionind,Urbind,income,incomeind,Ch1,Ch2,Ch3,Ch4))
df <- subset(safety, select = -c(segmentind,year,miles,milesind,night,nightind,pparkind,genderind,age,ageind,educind,regionind,Urbind,income,incomeind,Ch1,Ch2,Ch3,Ch4))
head(df)
```
...
rm(list=ls())
library(reticulate)
use_python("/Users/raynard/Library/r-miniconda-arm64/envs/r-reticulate/bin/python")
library(tensorflow)
library(keras)
library(splitTools)
library(dplyr)
...
```{r}
custom_logloss <- function(y_true, y_pred) {
  k <- backend()
  y_true <- k$cast(y_true, y_pred$dtype)
  loss <- -k$mean(y_true * k$log(y_pred + k$epsilon()), axis = as.integer(-1))
  return(loss)
}
```
...
# Compile model
model %>% compile(
  loss = custom_logloss, #this is already logloss
  optimizer = 'adam',
  metrics = c('accuracy')
)
