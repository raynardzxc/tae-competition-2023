...
```{r}
test1 <- read.csv("fulltestresults_rfor.csv")
test2 <- read.csv("fulltestresults_xgb3.csv")
...
vote2 <- soft_voting(test1, test2, weights = c(0.99, 0.01))
loss2 <- logloss(vote2)
loss2
...
# # With three predictions
# vote3 <- soft_voting(pred1, pred2, pred3, weights = c(0.4, 0.30, 0.30))
```
...
## Export
```{r}
write.csv(vote2, file = "../output/ensemble_4060.csv", row.names = FALSE)
```
